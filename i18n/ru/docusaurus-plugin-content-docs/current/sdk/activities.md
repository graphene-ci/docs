...
Тело выполняется в контейнере на машине. `machine.Command` через shell и
`nsenter` хоста входит в файловую систему и mount namespace хоста. На хосте
нужен util-linux; образу исполнителя shell не требуется. `exec.Cmd.Dir`
сохраняется, путь Docker socket под `/host` преобразуется в путь на хосте.
Созданные командой монтирования видны службам хоста, включая Docker. Объявляйте действия безусловно (не за ветками по рантайм-
