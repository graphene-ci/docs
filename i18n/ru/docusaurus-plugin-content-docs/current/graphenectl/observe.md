...
- **Scoped** — запрос вместе с записью: `graphenectl metrics run x --query
  ...
  Фильтр LogsQL должен закрываться: скобки сбалансированы вне строковых
  литералов, литералы завершены, без пайпа. `*) OR (…` закрыл бы скобки
  скоупа изнутри, поэтому отвергается как неверный аргумент до того, как
  его увидит backend; за скобками backend дополнительно получает namespace
  своим собственным extra-фильтром. `--query` не сочетается с `-f`:
  live-записи нельзя отфильтровать на языке backend, поэтому follow
  принимает флаги выборки (`--severity`, `--stream`, `--agent`,
  `--entity`, `--text`) и применяет их и к живому хвосту.
